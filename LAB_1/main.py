...
from Af_threshold import education_threshold, education_small_sample_threshold
from Af_logistic import education_logistic, education_small_sample
from Graph import *

#
# function stats the process of eduation based on a wi null array
def startnet():
    # start point
    w = [0, 0, 0, 0]
    w0 = 0
    ...
    E_array = list()
    k = 0
    i_list = [0,1,2,4,8,9,12]
    # while error is not 0 changing the wi array
    while E != 0:
        # educating the neural network and writing the result to log file
        E, w, w0 = education_small_sample_threshold(X, w, w0, x0, k, i_list)
        ...
        if k > 20:
            return 0
    # showing the plot
    error_plot(E_array, range(0, k), "Small set with threshold f(net) function")
